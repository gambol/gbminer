...
//        _last_time = curr_time;
    ...
    public CpuMiningWorker(int i_number_of_thread, long retry_pause, int priority) {
        ...
        for (int i = this._number_of_thread - 1; i >= 0; i--) {
            ...
            log.info("worker thread. init. i:{}", i);
        }
        ...
        if (i_work != null) {
            ...
            for (int i = this._number_of_thread - 1; i >= 0; i--) {
                ...
                try {
                    ...
                    log.error("exception. i:{}. work thread size:{}", i, this._workr_thread.length, e);
                }
...
//        this._last_hashed = this._num_hashed;
        for (int i = this._number_of_thread - 1; i >= 0; i--) {
            ...
            this._workr_thread[i] = new Worker(this);
            log.info("reinit worker thread. i:{}", i);
        }
        for (int i = this._number_of_thread - 1; i >= 0; i--) {
            //Set Work:
...
//            }
    ...
    public void stopWork() throws MinyaException {
        for (Worker t : _workr_thread) {
            if (t != null) {
                log.info("Worker: Killing thread ID: " + t.getId());
                t.interrupt();
//                log.info("Worker: thread ID: " + t.getId() + " isAlive() = " + t.isAlive());
