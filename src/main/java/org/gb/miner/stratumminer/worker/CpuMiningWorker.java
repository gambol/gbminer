...
import org.gb.miner.stratumminer.Console;
...
import org.gb.miner.stratumminer.hasher.Hasher;
...
public class CpuMiningWorker extends Observable implements IMiningWorker
{
    ...
    private class EventList extends ArrayList<IWorkerEvent>
    {
        ...
        {
            log.info("Nonce found! +"+((0xffffffffffffffffL)&i_nonce));
    ...
    private class Worker extends Thread implements Runnable
    {
        ...
        public void run()
        {
            ...
            try{
                ...
                while(true){
                    //log.info("CpuMiningWorker","run(), while loop");
                    for(long i=NUMBER_OF_ROUND-1;i>=0;i--){
                        byte[] hash = hasher.hash(work.header.refHex(), nonce);
                        //nonceのチェック
                        for (int i2 = hash.length - 1; i2 >= 0; i2--) {
                            //log.info("for loop in while", "iteration: i2= " + i2 );
                            if ((hash[i2] & 0xff) > (target[i2] & 0xff)){
                                log.info("hash[i2] & 0xff", "first condition, iteration: i= " + i + " i2= " + i2);
                                break;
                            }
                            if ((hash[i2] & 0xff) < (target[i2] & 0xff)){
                                //発見!
                                log.info("hash[i2] & 0xff", "second condition, iteration: i=" + i + " i2= " + i2);
                    ...
                    log.info("number_of_hashed", "" + number_of_hashed);
                    Thread.sleep(10L);
                    //running = false;
                }
                ...
                //Shutdownのハンドリング
                //running = false;
                log.info("Thread killed. Hashes= "+this.number_of_hashed);
                log.info("CpuMiningWorker", "Thread killed. Hashes= "+this.number_of_hashed);
...
//    }
    public void calcSpeedPerThread(long numOfHashes) {
        log.info("calcSpeedHashes: = ", "" + numOfHashes);
        long curr_time =  System.currentTimeMillis();
        double delta_time = Math.max(1,curr_time-this._last_time)/1000.0;
//        _last_time = curr_time;
        log.info("delta_time_double = ", "" + delta_time);
        log.info("calcSpeedPerThread","speed_calc_double= " + numOfHashes/delta_time);
        double speed_calc = ((double)numOfHashes/delta_time);
        log.info("speed_calc = ", "" + speed_calc);
    ...
    {
        log.info("Start doWork");
        log.info("CpuMiningWorker","Start doWork");
        if(i_work!=null){
        //if(this._exec!=null){
            //実行中なら一度すべてのワークを停止
            this.stopWork();
...
//                log.info("CpuMiningWorker","hashes: " + this._workr_thread[i].number_of_hashed);
            }
            log.info("CpuMiningWorker","Hashes: " + hashes);
            //ハッシュレートの計算
            _num_hashed = hashes;
            ...
            setChanged();
            notifyObservers(Notification.SPEED);
            log.info("Calculated "+ (_speed)+ " Hash/s");
            log.info("CpuMiningWorker","Calculated "+ (_speed)+ " Hash/s");
...
//        this._last_hashed = this._num_hashed;
        ...
        for(int i=this._number_of_thread-1;i>=0;i--){
            ...
            if (_workr_thread[i].isAlive() == false) {
                //log.info("isAlive()", " = false, about to  _workr_thread[i].start();");
                try {
...
//            }
    ...
    public void stopWork() throws MinyaException {
        for (Worker t : _workr_thread) {
            if (t != null) {
                log.info("LC", "Worker: Killing thread ID: " + t.getId());
                _console.write("Worker: Killing thread ID: " + t.getId());
                t.interrupt();
//                log.info("LC", "Worker: thread ID: " + t.getId() + " isAlive() = " + t.isAlive());
...
////        while (threads > 0) {
//            log.info("CpuMiningWorker", "Number of threads: " + threads);
...
//                // Preserve interrupt status
//                //Thread.currentThread().interrupt();
//                log.info("CpuMiningWorker", "Caught InterruptedException");
//                throw new MinyaException(e);
//            }
////        }
//        this._exec=null;
    ...
    {
        log.info("CpuMiningWorker","getNumberOfHash() = " + _tot_hashed);
        return _tot_hashed;
        //return 0;
    }
