...
public class MainActivity extends AppCompatActivity {
    ...
    public ServiceConnection mConnection = new ServiceConnection() {
        ...
        public void onServiceConnected(ComponentName name, IBinder service) {
            log.info("LC", "Main: onServiceConnected()");
            MinerService.LocalBinder binder = (MinerService.LocalBinder) service;
            mService = binder.getService();
            mBound=true;
            log.info("LC", "Main: Service Connected");
        }
    ...
    public void startMining() {
        log.info("LC", "Main: startMining()");
        mService.startMiner();
    }

    public void stopMining()
    {
        log.info("LC", "Main: stopMining()");
        mService.stopMiner();
...
//    int temperature;
    ...
    final Runnable rConsole = new Runnable() {
        public void run() {
            //log.info("LC", "StatusActivity:updateConsole:"+mService.console.getConsole());
    ...
    final Runnable rSpeed = new Runnable() {
        public void run() {
            // log.info("LC", "StatusActivity:updateSpeed");
    ...
    final Runnable rAccepted = new Runnable() {
        public void run() {
            // log.info("LC", "StatusActivity:updateAccepted");
    ...
    final Runnable rRejected = new Runnable() {
        public void run() {
            // log.info("LC", "StatusActivity:updateRejected");
    ...
    final Runnable rStatus = new Runnable() {
        public void run() {
            //  log.info("LC", "StatusActivity:updateStatus");
    ...
    final Runnable rBtnStart= new Runnable() {
        public void run() {
            // log.info("LC", "StatusActivity: Miner stopped, changing button to start");
...
//                firstRunFlag = false;
    ...
    final Runnable rBtnStop= new Runnable() {
        public void run() {
            //log.info("LC", "StatusActivity: Miner stopped, changing button to stop");
            Button b = (Button) findViewById(R.id.status_button_startstop);
    ...
    Thread updateThread = new Thread () {
        public void run() {
            log.info("LC", "StatusActivity: Update thread started");
            ...
            while (mBound==false)
            {
                try {
                    ...
                    log.info("LC", "StatusActivity:updateThread: Interrupted");
                }
            }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        log.info("LC", "Main: in onCreate()");
//        setTitle("StratumMiner");

        //setContentView(R.layout.activity_status);
        log.info("LC", "Status: onCreate");
...
//                                                     b.setEnabled(false);
//                                                     b.setClickable(false);
//                                                     //log.info("MainActivity","stopMining: baseThreadCount = " + baseThreadCount);
//                                                     CpuMiningWorker worker = (CpuMiningWorker)mService.imw;
//                                                     ThreadStatusAsyncTask threadWaiter = new ThreadStatusAsyncTask();
//                                                     threadWaiter.execute(worker);
                                                 }
...
//    }
    ...
    public class ThreadStatusAsyncTask extends AsyncTask<CpuMiningWorker,Integer,Boolean> {
        ...
        protected Boolean doInBackground(CpuMiningWorker... params) {
            ...
            while (params[0].getThreadsStatus()) {
                ...
                if (deltaTime>15.0) {
                    log.info("AsyncTask","Still Waiting");
    ...
    protected void onStop() {
        ...
        log.info("LC", "Main: in onStop()");
        try {
            unbindService(mConnection);
        } catch (RuntimeException e) {
            log.info("LC", "RuntimeException:"+e.getMessage());
...
//                //log.info(TAG, logDateFormat.format(new Date()) + str);
//
//            }
//        });
//    }
...
//            log.info(TAG, "Usage:  java Miner [URL] [USERNAME:PASSWORD] [THREADS] [THROTTLE] [SCANTIME] [RETRYPAUSE]");
//            return;
//        }
//
